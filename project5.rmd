```{r}
#read data
info <- read.table('lt1720uk.dat', header=TRUE)
death <- read.table('death1722uk.dat', header=TRUE)

population_m <- info$mpop17
population_f <- info$fpop17
death_m <- info$mm
death_f <- info$mf
modifier <- death$d
```

```{r}
#Q1
predicted_death_week <- function(population_m, population_f, dr_m, dr_f, mod){
  
  q_m <- 1 - exp(-dr_m/52)
  q_f <- 1 - exp(-dr_f/52)
  pop_m <- population_m
  pop_f <- population_f
  death_count_week <- rep(0,length(mod)) 
  
  for (i in 1:length(mod)){
    d <- mod[i] # modifier in week i
    death_m <- d*0.9885*q_m*pop_m
    death_f <- d*0.9885*q_f*pop_f
    death_count_week[i] <- sum(death_m + death_f)
    
    pop_m[1] <- pop_m[1] - death_m[1]
    pop_f[1] <- pop_f[1] - death_f[1]
    
    for (i in 2:length(pop_m)){
      pop_m[i] <- (pop_m[i] - death_m[i])*(51/52) + pop_m[i-1]/52
      pop_f[i] <- (pop_f[i] - death_f[i])*(51/52) + pop_f[i-1]/52
    }
    
  }
  
  death_count_week 
}

```


```{r}
test <- predicted_death_week(population_m, population_f, death_m, death_f, modifier)
(sum(test[1:156])-sum(death$deaths[1:156]))
```

```{r}
#Q2
new_m <- info$mpop20
new_f <- info$fpop20

real_new_all <- death$deaths[157:305]
new_all <- predicted_death_week(new_m, new_f, death_m, death_f, modifier[157:305])
diff_all <- sum(new_all - real_new_all)
excess_death_all <- real_new_all - new_all

real_new <- death$deaths[157:208]
new <- predicted_death_week(new_m, new_f, death_m, death_f, modifier[157:208])
diff <- sum(new - real_new)
diff
```

```{r}
#Q3
week <- c(1:52)
plot(week, real_new, col = 'blue', xlab = "weeks", ylab = "number of death")
lines(week, new, col = 'red')
legend(x = 'topright', legend=c("observed", "predicted"),
       col=c("blue", "red"), inset = 0.05, lty = 3:1 ,cex=1.2, border = "black",
       box.lty=0, bg = rgb(1, 0, 0, alpha = 0.15))
```

```{r}
#Q4
excess_death <- real_new - new
cum_excess <- cumsum(excess_death)
#plot(week, cum_excess, type = 'p')
plot(week, cum_excess, type = "b", pch = 18, col = "red", xlab = "weeks", ylab = "excess death")
```

```{r}
#Q5
library('rjags')
set.seed(1)

jags_params <- c("mu", "rho", "k")
delete <- c(51, 52, 53, 105, 106)
for (i in delete){
  excess_death_all[i] <- NA
}

jags_data <- list(x = excess_death_all, N = length(excess_death_all))
model_jags <- jags.model("model.jags", data = jags_data) # n.chains=2)
sample <- jags.samples(model_jags, jags_params, n.iter=10000)
#str(sample)
#dic.samples(model_jags,n.iter=10000)
```

```{r}
sample_coda <- coda.samples(model_jags, jags_params, n.iter=10000)
summary(sample_coda)
str(sample_coda)
#plot(sample_coda$rho)
```

